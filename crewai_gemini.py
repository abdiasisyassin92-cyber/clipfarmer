...
def run_with_backoff(max_retries=5, base_wait=2):
    """
    Runs crew.kickoff() with exponential backoff on 503 / overload errors.
    Waits: 2s, 4s, 8s, 16s, 32s between retries before giving up.
    """
    attempt = 0
    while attempt <= max_retries:
        try:
            ...
            error_str = str(e)
            is_overload = any(code in error_str for code in ["503", "UNAVAILABLE", "overloaded", "high demand"])

            if is_overload and attempt < max_retries:
                wait = base_wait ** (attempt + 1)
                print(f"\n[RETRY] Gemini overloaded (attempt {attempt + 1}/{max_retries}). "
